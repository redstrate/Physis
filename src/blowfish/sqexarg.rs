...
///
/// ```
/// # use physis::blowfish::SqexArgBlowfish;
/// let key = b"abcdefgh";
/// let data = b"foobar  ";
///
/// let fish = SqexArgBlowfish::new(key);
/// let encrypted = fish.encrypt(data).unwrap();
/// let decrypted = fish.decrypt(&encrypted).unwrap();
/// # assert_eq!(data, &decrypted[..])
/// ```
pub struct SqexArgBlowfish {
    p: [u32; 18],
    s: [[u32; 256]; 4],
}

impl SqexArgBlowfish {
...
mod tests {
    ...
    fn test_encrypt_decrypt() {
        let blowfish = SqexArgBlowfish::new(b"test_case");
